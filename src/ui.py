...
def screen_access_short(user: User) -> str:
    ...
    return (
        ...
        f"<i>свобода без границ</i>\n\n"
        f"<blockquote><b>◆  статус · {badge}</b>\n{_esc(detail)}</blockquote>\n\n"
        f"Ссылка профиля — следующим сообщением.\n"
        f"Нажмите на неё и удерживайте → «Копировать».\n"
        f"В Happ: «+» → «Добавить подписку» → вставить."
    )
...
def screen_help() -> str:
    ...
    return (
        ...
        f"<b>2.</b>  Скопируйте ссылку из следующего сообщения\n"
        f"<b>3.</b>  Happ → «+» → «Добавить подписку» → вставить</blockquote>\n\n"
